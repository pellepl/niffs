...
TEST(func_creat) {
  ...
  TEST_CHECK_EQ(niffs_create(&fs, 0, _NIFFS_FTYPE_FILE, 0), ERR_NIFFS_NULL_PTR);
  TEST_CHECK_EQ(niffs_create(&fs, "test", _NIFFS_FTYPE_FILE, 0), NIFFS_OK);
  TEST_CHECK_EQ(niffs_create(&fs, "test", _NIFFS_FTYPE_FILE, 0), ERR_NIFFS_NAME_CONFLICT);
...
TEST(func_creat_full) {
  ...
  for (i = 0; i < (fs.sectors-1) * fs.pages_per_sector; i++) {
    char fname[16];
    sprintf(fname, "t%i", i);
    res = niffs_create(&fs, fname, _NIFFS_FTYPE_FILE, 0);
    TEST_CHECK_EQ(res,  NIFFS_OK);
  }

  res = niffs_create(&fs, "overflow", _NIFFS_FTYPE_FILE, 0);
...
TEST(func_delete) {
  ...
  TEST_CHECK_EQ(niffs_create(&fs, "moo", _NIFFS_FTYPE_FILE, 0), NIFFS_OK);
...
TEST(func_open) {
  ...
  res = niffs_create(&fs, "test", _NIFFS_FTYPE_FILE, 0);
  TEST_CHECK_EQ(res,  NIFFS_OK);
...
TEST(func_modify_ohdr) {
  ...
  res = niffs_create(&fs, "modify", _NIFFS_FTYPE_FILE, 0);
  TEST_CHECK_EQ(res,  NIFFS_OK);
...
TEST(func_truncate) {
  ...
  res = niffs_create(&fs, "trunc", _NIFFS_FTYPE_FILE, 0);
  TEST_CHECK_EQ(res,  NIFFS_OK);
...
TEST(func_rename) {
  ...
  res = niffs_create(&fs, "orig", _NIFFS_FTYPE_FILE, 0);
  TEST_CHECK_EQ(res,  NIFFS_OK);
  ...
  res = niffs_create(&fs, "new2", _NIFFS_FTYPE_FILE, 0);
  TEST_CHECK_EQ(res,  NIFFS_OK);
...
TEST(func_gc_big_hog) {
  ...
  res = niffs_create(&fs, "bighog", _NIFFS_FTYPE_FILE, 0);
  TEST_CHECK_EQ(res,  NIFFS_OK);
...
TEST(func_check_aborted_delete) {
  ...
  res = niffs_create(&fs, "undel", _NIFFS_FTYPE_FILE, 0);
  TEST_CHECK_EQ(res,  NIFFS_OK);
  u32_t len = _NIFFS_SPIX_2_PDATA_LEN(&fs, 0) + _NIFFS_SPIX_2_PDATA_LEN(&fs, 1) * (2*fs.pages_per_sector-1);
  ...
  res = niffs_create(&fs, "noorphan", _NIFFS_FTYPE_FILE, 0);
  TEST_CHECK_EQ(res,  NIFFS_OK);
...
TEST(func_check_orphans) {
  ...
  // create one big file two sectors big
  res = niffs_create(&fs, "orphan", _NIFFS_FTYPE_FILE, 0);
  TEST_CHECK_EQ(res,  NIFFS_OK);
  u32_t len = _NIFFS_SPIX_2_PDATA_LEN(&fs, 0) + _NIFFS_SPIX_2_PDATA_LEN(&fs, 1) * (2*fs.pages_per_sector-1);
...
TEST(func_check_aborted_append) {
  ...
  // create file
  res = niffs_create(&fs, "abortapp", _NIFFS_FTYPE_FILE, 0);
  TEST_CHECK_EQ(res,  NIFFS_OK);
...
TEST(func_lin_alloc_virgin) {
  ...
  res = niffs_linear_find_space(&fs, 1, &start_sect);
  TEST_CHECK_EQ(res, NIFFS_OK);
  TEST_CHECK_EQ(start_sect, fs.sectors);

  res = niffs_linear_find_space(&fs, fs.lin_sectors, &start_sect);
  TEST_CHECK_EQ(res, NIFFS_OK);
  TEST_CHECK_EQ(start_sect, fs.sectors);

  res = niffs_linear_find_space(&fs, fs.lin_sectors+1, &start_sect);
  TEST_CHECK_EQ(res, ERR_NIFFS_LINEAR_NO_SPACE);

  return TEST_RES_OK;
} TEST_END

TEST(func_lin_alloc_mknod) {
  u32_t start_sect;
  int res = NIFFS_format(&fs);
  TEST_CHECK_EQ(NIFFS_mount(&fs), NIFFS_OK);
  int fd;

  fd = NIFFS_mknod_linear(&fs, "1_1", 0);
  TEST_CHECK_GE(fd, NIFFS_OK);
  NIFFS_close(&fs, fd);
  res = niffs_linear_find_space(&fs, 1, &start_sect);
  TEST_CHECK_EQ(res, NIFFS_OK);
  TEST_CHECK_EQ(start_sect, fs.sectors+1);

  fd = NIFFS_mknod_linear(&fs, "2_1", 0);
  TEST_CHECK_GE(fd, NIFFS_OK);
  NIFFS_close(&fs, fd);
  res = niffs_linear_find_space(&fs, 1, &start_sect);
  TEST_CHECK_EQ(res, NIFFS_OK);
  TEST_CHECK_EQ(start_sect, fs.sectors+1+1);

  fd = NIFFS_mknod_linear(&fs, "3_8", fs.sector_size*8-1);
  TEST_CHECK_GE(fd, NIFFS_OK);
  NIFFS_close(&fs, fd);
  res = niffs_linear_find_space(&fs, 1, &start_sect);
  TEST_CHECK_EQ(res, NIFFS_OK);
  TEST_CHECK_EQ(start_sect, fs.sectors+1+1+8);

  fd = NIFFS_mknod_linear(&fs, "4_1", fs.sector_size/2);
  TEST_CHECK_GE(fd, NIFFS_OK);
  NIFFS_close(&fs, fd);
  res = niffs_linear_find_space(&fs, 1, &start_sect);
  TEST_CHECK_EQ(res, NIFFS_OK);
  TEST_CHECK_EQ(start_sect, fs.sectors+1+1+8+1);

  fd = NIFFS_mknod_linear(&fs, "5_lots", fs.lin_sectors);
  TEST_CHECK_GE(fd, ERR_NIFFS_LINEAR_NO_SPACE);
...
TEST(func_lin_write) {
  ...
  const u32_t len = fs.sector_size*5;
  u8_t *data = niffs_emul_create_data("linear", len);
  TEST_CHECK(data);

  fd = NIFFS_mknod_linear(&fs, "linear", 0);
  TEST_CHECK_GE(fd, NIFFS_OK);
  res = NIFFS_write(&fs, fd, data, len/2);
  TEST_CHECK_EQ(res, len/2);

  res = NIFFS_write(&fs, fd, &data[len/2], len/2);
  TEST_CHECK_EQ(res, len/2);
